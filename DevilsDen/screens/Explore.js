...
const ExploreScreen = ({ navigation, route }) => {
    ...
    const getUserContacts = () => {
      ...
      const unsubscribe = onSnapshot(q, async (snapshot) => {
        ...
        const contactDetails = contactsSnap.docs
          .filter((d) => {
            const contactGraduationYear = d.data().graduationYear;
            return (
              contactGraduationYear >= userGraduationYear - 2 &&
              contactGraduationYear <= userGraduationYear + 2
            );
          })
          .map((d) => ({
  ...
  return (
    <Fragment>
      ...
      <View
        style={{ backgroundColor: "#E5EAFD", flex: 1, alignItems: "center" }}
      >
        <Image
          source={require("../assets/Explore_hero.jpg")}
          style={{
            width: imageWidth,
            height: 270,
            marginBottom: 15,
        ...
        />
        <Text style={{ marginVertical: 20, fontWeight: "800" }}>
          SWIPE to find who is your FRIEND
        </Text>
